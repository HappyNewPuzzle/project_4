...
class BlogOllamaClient:
    ...
    def generate_post(
        ...
        try:
            # 로컬 모델의 첫 실행은 오래 걸릴 수 있어 제한 시간을 5분으로 둡니다.
        ...
        # 최상위 응답이 객체가 아니면 message를 읽기 전에 안전하게 중단합니다.
        if not isinstance(payload, dict):
            raise OllamaClientError("Ollama 서버 응답 형식이 올바르지 않습니다.")

        # 정상 응답은 message.content 안에 모델이 만든 JSON 문자열을 담습니다.
        content = payload.get("message", {}).get("content")
        if not isinstance(content, str) or not content.strip():
            raise OllamaClientError("Ollama가 비어 있거나 올바르지 않은 응답을 반환했습니다.")

        try:
            # 모델이 만든 구조화 JSON 문자열을 글 검증에 사용할 딕셔너리로 바꿉니다.
            result = json.loads(content)
        except json.JSONDecodeError as exc:
            raise OllamaClientError("Ollama의 글 응답을 JSON으로 해석할 수 없습니다.") from exc

        # 예상과 다른 배열이나 문자열이 오면 후속 처리를 중단합니다.
        if not isinstance(result, dict):
            raise OllamaClientError("Ollama의 글 응답 형식이 올바르지 않습니다.")
        # 검증 가능한 결과를 공통 글 생성 모듈로 반환합니다.
        return result
