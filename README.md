...
프로그램을 실행하면 네 가지 방식을 선택할 수 있습니다.

```text
1. ChatGPT용 프롬프트 만들기
2. Ollama 로컬 모델로 글 생성
3. OpenAI API로 글 생성
4. AI 없이 네이버 내 템플릿 채우기
```

4번은 AI와 GPU를 사용하지 않습니다. 네이버 공식 `내 템플릿`에 미리
배치한 구매 스티커는 그대로 유지하고, 고유 표시만 사진과 직접 작성할
안내 문구로 교체합니다.
...
자동 배치를 시작하면 사용할 네이버 계정 프로필을 선택합니다. `새 계정 추가`를
고른 뒤 알아보기 쉬운 별칭을 입력하고, 열린 브라우저에서 해당
네이버 계정으로 최초 한 번 로그인합니다. 다음 실행부터는 목록에서 그
별칭만 선택하면 저장된 로그인 상태를 재사용합니다.

계정별 로그인 쿠키는 `data/naver_browser_profiles/계정별칭` 폴더에 서로
분리해 저장합니다. 네이버 아이디와 비밀번호는 프로그램이 입력받거나
저장하지 않으며, 해당 폴더도 GitHub에 올라가지 않습니다. 이전 버전의
`data/naver_browser_profile` 로그인 상태는 `기존 기본 계정`으로 계속
사용할 수 있습니다.

Ollama·OpenAI API로 글을 생성하거나 4번 템플릿 초안을 만든 뒤 다음
질문에 `y`를 입력하면 네이버 편집 자동화를 시작합니다.
...
`data/naver_browser_profiles/계정별칭`에 저장됩니다. 일반 Chrome 프로필과
충돌하지 않는 전용 프로필입니다.
...
먼저 네이버 글쓰기 화면에서 구매 스티커와 아래 표시를 각각 별도 한 줄로
배치한 뒤 `내 템플릿`으로 저장합니다. 표시 문단에는 다른 글자를 함께
입력하지 마세요.

상품 리뷰 템플릿 이름은 `상품리뷰_자동`이며 필요한 표시는 다음과 같습니다.
...
가게 리뷰 템플릿 이름은 `가게리뷰_자동`이며 필요한 표시는 다음과 같습니다.
...
프로그램에서 4번을 선택하고 자동 배치를 시작하면 상품은 배송·포장 사진
수를, 가게는 외부·내부·메뉴판 사진 수를 차례로 묻습니다. 나머지는 마지막
상세·사용 또는 음식 구간에 자동 배정됩니다. 브라우저가 열리면 안내된
`내 템플릿`을 직접 한 번 불러온 뒤 콘솔에서 Enter를 누릅니다. 프로그램은
모든 표시가 있는지 먼저 확인한 다음 사진과 작성 공간으로 교체합니다.
